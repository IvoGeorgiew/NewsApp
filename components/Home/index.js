...
export default function Home() {
  ...
  useEffect(() => {
    axios //https://newsapi.org/v2/top-headlines?country=us&apiKey=API_KEY
  ...
  function searchCharacter() {
    if (search) {
      ...
      axios
        .get(
          `${APIALLNEWS}${search}&sortBy=popularity&apiKey=${APIKEY}&pageSize=10`
        )
        .then((response) => setData(response.data.articles))
        .then(setSearch(""))
        .catch((error) => console.error(error))
